 ...
 * Today's reading.
 *
 * Each card is drawn from its own child seed and the two are drawn *independently* rather
 * than from a shrinking pool: the deck in the game is one card per arcana, so the same card
 * coming up in both positions is possible and should not be silently prevented. The
 * orientation is a third draw, so it cannot correlate with which card was taken.
 *
 * The salting is per position as well as per purpose, so adding a position later cannot
 * shift the cards the existing two are dealt.
 */
export function readTarot(seed: Seed): TarotReading {
	const draws = TAROT_POSITIONS.map((position): TarotDraw => {
		const card = seed.derive(`tarot-card-${position}`).pick(TAROT);
	...
	// The map is over a two-element tuple, so the result is a pair; the cast is what says so.
	return draws as unknown as TarotReading;
}
