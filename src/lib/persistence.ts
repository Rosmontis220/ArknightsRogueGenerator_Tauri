 ...
 * before the first paint so the window never flashes the wrong theme, and the date each
 * 今日运势 practice was last performed, which is a view flag rather than a setting — see
 * {@link readFortuneOn}.
 *
...
/**
 * The 今日运势 practices, one key each.
 *
 * The toss key keeps its original name so that a browser profile written before 塔罗牌
 * existed still opens on the day's coins rather than asking again.
 */
const TOSS_DATE_KEY = 'arg:tossed-on';
const TAROT_DATE_KEY = 'arg:tarot-on';

/** The two practices on the home screen's 今日运势 card. */
export type FortunePractice = 'toss' | 'tarot';

const FORTUNE_KEYS: Readonly<Record<FortunePractice, string>> = {
	toss: TOSS_DATE_KEY,
	tarot: TAROT_DATE_KEY
};
...
/**
 * The day a practice was last performed, or `null`.
 *
 * The result is fixed for one ID on one day, so the screen only has to remember *that* it
 * was asked, never what came up — it is derived again from the same `(ID, date)` pair. That
 * is what keeps each of these to one short string.
 *
 * Here rather than in the state file because it is a view flag with a one-day lifetime, not
 * a setting: it is worth one click if it is ever lost, and putting it in the document would
 * cost a schema version and a migration to carry it. The state file is for what the user
 * chose; this is for what the user has already been shown.
 *
 * Never throws, for the reason {@link readThemeMirror} gives.
 */
export function readFortuneOn(practice: FortunePractice): string | null {
	try {
		return globalThis.localStorage?.getItem(FORTUNE_KEYS[practice]) ?? null;
	} catch {
		return null;
	}
}
...
/**
 * Records the day a practice was performed, so that reopening the app does not ask again.
 *
 * Nothing clears it. The home page compares it against the current date, so it stops
 * matching on its own at midnight and the next draw overwrites it — a stale value is
 * indistinguishable from no value at all.
 */
export function writeFortuneOn(practice: FortunePractice, date: string): void {
	try {
		globalThis.localStorage?.setItem(FORTUNE_KEYS[practice], date);
	} catch {
		// Storage unavailable or full; the draw still works, it is just asked again.
	}
}

/** The day the 投钱问路 toss was last made. Shorthand for {@link readFortuneOn}. */
export function readTossedOn(): string | null {
	return readFortuneOn('toss');
}

/** Records the day the toss was made. Shorthand for {@link writeFortuneOn}. */
export function writeTossedOn(date: string): void {
	writeFortuneOn('toss', date);
}
...
export function normalizeState(raw: Partial<AppState>): AppState {
	...
	return {
		...
		sound: normalizeSound(base.sound, raw.sound),
...
/**
 * The volume, clamped to the range the slider can express.
 *
 * The spread would carry a stored `volume` through unexamined, and a value outside 0–100
 * reaches `HTMLMediaElement.volume`, which throws above 1. A document edited by hand, or
 * written by a future release with a wider range, would then fail at the one moment a sound
 * is supposed to play. Non-finite values are rejected for the same reason: `NaN` fails
 * every comparison, so it would survive a naive clamp.
 */
function normalizeSound(base: SoundState, raw: SoundState | undefined): SoundState {
	...
	return { volume: Math.min(100, Math.max(0, Math.round(volume))) };
}
