<script lang="ts">
	...
	import { TOSS_SOUNDS, playSound } from '$lib/sound';
	...
	function reveal(practice: FortunePractice): void {
		...
		if (practice === 'toss') {
			...
			// The coins are already decided — they are derived from the same `(ID, date)` pair
			// — so this is the moment the reading lands, and the only moment the sound belongs
			// to. It also sounds when a finished toss is re-opened from 已抽, because that
			// re-opens the same reading; that is deliberate, and it is the only way to hear the
			// sound again without waiting for tomorrow.
			playSound(TOSS_SOUNDS[verdict], appState.state.sound.volume);
		} else {
			tarotOn = today;
